...
void ComputeScenario(SCENARIO* Scenario, const char* OutputFileName)
{
	...
	for (unsigned int Step = 0; Step < (unsigned int)(Scenario->MAX_TIME / Scenario->TIME_STEP); Step++)
	{
		...
		else if (SourceActive)
		{
			...
			int Y;
			#pragma omp parallel for
			for (Y = SourceRegion.TopLeft.Y; Y < SourceRegion.BottomRight.Y; Y++)
				for (int X = SourceRegion.TopLeft.X; X < SourceRegion.BottomRight.X; X++)
					if (IsSource(Scenario, X, Y, 1))
						A[AT(X, Y)] = B[AT(X, Y)] = C[AT(X, Y)] = 0;
		}
	...
	printf("\n    Output has %lu frames (%lux%lu)\n", (unsigned int)(Scenario->MAX_TIME / Scenario->TIME_STEP)/Scenario->SAVE_TIME,
